...
public:
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
        ...
        if (find(key) != end()) { // do not allow insertion of duplicate key
...
private:
    ...
    int diameter(Node<K, V> *node) const {
        ...
        return std::max(2 + height(node->left) + height(node->right), std::max(diameter(node->left), diameter(node->right)));
    }
    ...
    size_t width(Node<K, V> *node, size_t level) const {
        ...
        if (level == 1) {
            size_t node_cnt = 0;
            node_cnt += node->left != NULL ? 1 : 0;
            node_cnt += node->right != NULL ? 1 : 0;
            return node_cnt;
        }
